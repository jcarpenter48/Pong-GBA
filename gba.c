...
void drawRectDMA(int row, int col, int width, int height, volatile u16 color) {
  ...
  for (int i = 0; i < height; ++i) {
      DMA[3].src = &color;
      //DMA[3].dst = &videoBuffer[OFFSET(col + i, row, WIDTH)];
      DMA[3].dst = &videoBuffer[OFFSET(col + i, row, 240)]; //we know width is 240 anyways, less confusing
...
void drawImageDMA(int row, int col, int width, int height, const u16 *image) {
  ...
  for (int i = 0; i < height; ++i) {
      ...
      DMA[3].dst = &videoBuffer[OFFSET(col + i, row, 240)];
